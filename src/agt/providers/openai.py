"""OpenAI provider adapter hidden behind the internal LLMProvider protocol."""
...
@dataclass(slots=True)
class _HTTPOpenAIResponsesModel:
    ...
    def _parse_response(payload: dict[str, object]) -> _ModelResponse:
        ...
        content = output_text.strip() if isinstance(output_text, str) else ""
        ...
        if not content:
            ...
            if isinstance(output, list):
                ...
                for item_obj in output_items:
                    ...
                    content_blocks_obj = item.get("content")
                    if not isinstance(content_blocks_obj, list):
                        continue
                    content_blocks = cast(list[object], content_blocks_obj)
        ...
        if not content:
            raise ProviderResponseError("OpenAI response missing text output")
    ...
    def invoke(self, prompt: str) -> _ModelResponse:
        try:
            for attempt in Retrying(
                ...
                with attempt, httpx.Client(timeout=self.timeout, base_url=self._base_url) as client:
                    ...
                    if not isinstance(data, dict):
                        raise ProviderResponseError("OpenAI response payload must be a JSON object")
            ...
            raise ProviderResponseError(f"OpenAI returned HTTP {exc.response.status_code}") from exc
        except httpx.NetworkError as exc:
            raise ProviderTimeoutError("OpenAI network request failed") from exc

        raise ProviderResponseError("OpenAI request failed after retries")
    ...
    def bind_tools(self, tools: Sequence[object]) -> _HTTPOpenAIResponsesModel:
        return _HTTPOpenAIResponsesModel(
...
class OpenAIProvider(XAIProvider):
    """Adapter around the OpenAI Responses API via HTTPX."""
