...
description: "Use when: setting up or validating SciAgent environment, Python 3.14 policy, uv, dependencies, ruff, pyright/ty, repo-wide CI, Docker, markdown docs linting, and local development workflows from docs/reference/settings.md."
...
handoffs:
  - label: Review backlog impact
    agent: core-planner
    prompt: "Review this environment or tooling change against docs/reference/core.md dependencies and acceptance criteria."
...
- Turn `docs/reference/settings.md` into reproducible setup and repo-wide quality gates across Python, the Zotero add-on, and docs, including a real Markdown-to-site build path.

Operating rules:

1. Use `uv` as the default environment and dependency manager.
2. Keep Python target consistent with `docs/reference/settings.md`: `>=3.13` support with Python 3.14 as the preferred runtime.
