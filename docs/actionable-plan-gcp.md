...
> **Prerequisite.** Every box in `docs/actionable-plan.md` (P10) is checked. If
> it isn't, stop and do that first. This plan assumes:
> - The cheap LLM (DeepSeek or equivalent) is wired and smoke-tested locally.
> - The `Dockerfile` honors `$PORT`.
> - `.dockerignore` exists and the image is < 300 MB.
...
- [ ] **M4.4** Smoke test `/health`:
  ...
  Expect 200 JSON. Check for the `sciagent_startup` log line (C2 from prereq
  plan) — it should show `llm_provider=openai-compatible` and
...
- [ ] Image size < 300 MB? (check in M9.5)
...
| `.dockerignore` misses a heavy file | Low | Check `docker images` size after M4.1. If > 300 MB, inspect with `docker history sciagent:latest`. |
...
- Prerequisite: `docs/actionable-plan.md` (P10) complete? **[ ]**
- Current focus: **M0 — GCP Account & Local Tooling**
- Current next implementation target: **M0.1** *(only after P10 is done)*
- Last completed: *(none)*

### Phase Tracker

- [ ] **P10 prereqs done** — gate: do not proceed past this line until checked
...
2. Do not start M0 until the prereq gate above is checked.
...
- `docs/actionable-plan.md` — P10 prereqs (must be done first).
